...
%                     ,(ss-1)*(N_Rx-1) + nn - 1));
                if plotfig == 1
                    figure(1);plot(LoadPhase{(ss-1)*(N_Rx-1) + nn - 1});hold on;
                end
...
ChosenIndex = MADInd(pkssumIndex(1:ChosenNum)); % 选取三条
        ...
        if dataPerson(ii) < 3
            for ss = 1:SelNum
                SelPhase = [LoadPhase{ChosenIndex(ss + 1)};SelPhase];
            end
        else
        for ss = 1:3
                SelPhase = [LoadPhase{ChosenIndex(ss + 1)};SelPhase];
        ...
        % p_phase > mean(p_phase) - sqrt(var(p_phase))
...
%         if Interval(kk,1) < 0.5*fs(ii)*60/MaxBPM || Interval(kk,2) < 0.5*fs(ii)*60/MaxBPM
...
%         timepeakNum = sum((p/max(p) >= 0.08) + (w >= 1/3*fs(ii)*60/MaxBPM) == 2);
        ...
        Index = Index(w_phase >= 0.6*fs(ii)*60/MaxBPM);
        BPM_in_Time = 60/(length(SelPhase)/fs(ii)/timepeakNum);
%         if dataPerson(ii) == 1
