...
ChosenIndex = MADInd(pkssumIndex(1:ChosenNum)); % 选取三条
        ...
        [timepeak, Index, w, p] = findpeaks(SelPhase); % 时域上峰值
%         timepeakNum = length(timepeak);
%         timepeakNum = sum(timepeak >= mean(SelPhase) + 0.05*sqrt(var(SelPhase)));
w = w(timepeak >= mean(SelPhase) + 0.05*sqrt(var(SelPhase)));
p = p(timepeak >= mean(SelPhase) + 0.05*sqrt(var(SelPhase)));
...
%         BPM(ii,1:dataPerson(ii)) = sort(BPM(ii,1:dataPerson(ii)));
    ...
    % 创建一个名为data.txt的文本文件
    fileID = fopen(strcat('CompetitionData2/',name), 'wt');
    % 向文件中写入数据
    for ii = 1:dataNum
        Outputdata = BPM(ii,:);
