...
if __name__ == "__main__":
    ...
    ## DrawFig5 in ContestDocs
    csiSamp = samp.CSI_s[41]
    sig = samp.CSI_s[41][2,0,16,2401:] * np.conj(samp.CSI_s[41][0,0,16,2401:])
    wa = samp.Wave[41][0]['Wave']
    plt.figure()
    plt.subplot(211)
    plt.plot(wa[1500:3000],'-b')
    plt.grid()
    plt.legend(["Measured Breath Wave"])
    plt.xlabel('Time')
    plt.ylabel('Amp.[mV]')
    plt.subplot(212)
    plt.plot(np.angle(sig)*180/np.pi,'-r')
    plt.ylim([0,180])
    plt.grid()
    plt.xlabel('Time')
    plt.ylabel('Pha.[Deg.]')
    plt.legend(["CSI Differential Phase Signal"])
    plt.show()
