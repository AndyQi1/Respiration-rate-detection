...
for SampleNum = 1%:length(X)
    ...
    for ii = 1%:length(N_T)
        ...
        data = real + 1j*imag;
        datavar = var(data);
        datavar = reshape(datavar,3,[]);
        [~,datavar_sorted] = sort(sum(datavar),'descend');
        ChosenCarrierNum = N_Sc/2;
        ChosenCarrier = datavar_sorted(1:ChosenCarrierNum);
        if plotfig == 1
        for dd = 1:length(ChosenCarrier)
            figure(1); plot(abs(data(:,ChosenCarrier(dd))));hold on;
        end
        end
        phase = zeros(N_T(ii),ChosenCarrierNum*(N_Rx - 1));
        
        len = N_T(ii);
        Precyclix = round(3/(T_Dur(ii)/len)); % 保护区间（5s）
        WindowLen = round(len/(T_Dur(ii)*(MaxBPM + MinBPM)/120)); % 计算滑动窗口
        LoadPhase = cell(1,ChosenCarrierNum*(N_Rx - 1));
        pr = zeros(1,ChosenCarrierNum*(N_Rx - 1));
        for ss = 1:ChosenCarrierNum
            for nn = 2:N_Rx
                phase(:,(ss-1)*(N_Rx-1) + nn - 1) = 180*angle(conj(data(:,(ChosenCarrier(ss)-1)*N_Rx + 1))...
                    .*data(:,(ChosenCarrier(ss)-1)*N_Rx + nn))/pi; % 转为角度制
                ...
                if plotfig == 1
            figure(2); plot(phase([StartPoint:EndPoint],(ss-1)*(N_Rx-1) + nn - 1));hold on;
        end
...
%         [~,ChosenIndex] = max(pr);
        ChosenStart = 3;
        ChosenNum = 5;
        MAD = zeros(1, ChosenCarrierNum*(N_Rx - 1));
        for mm = 1:ChosenCarrierNum*(N_Rx - 1)
        ...
        MADInd(ChosenStart:ChosenStart + ChosenNum - 1);
...
ChosenIndex = MADInd(pkssumIndex(1:ChosenNum)); % 选取三条
        FinalSelPhase = [];
        SelNum = 1;
        SelCarrier = floor(ChosenIndex(1:SelNum)/(N_Rx - 1));
        if dataPerson(ii) < 3
            for ss = 1:N_Rx - 1
                FinalSelPhase = [LoadPhase{SelCarrier + ss};FinalSelPhase];
            end
        else
        for ss = 1:SelNum
