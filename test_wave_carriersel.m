...
%         [~,ChosenIndex] = max(pr);
        ...
        MAD = zeros(1, N_Sc);
        for mm = 1:N_Sc
            for mmm = 1:N_Rx*(N_Rx - 1)/2
                MAD(mm) = mad(LoadPhase{N_Rx*(N_Rx - 1)/2*(mm - 1) + mmm},1) + MAD(mm);
            end
        end
...
%         ChosenIndex = MADInd(pkssumIndex(1:ChosenNum)); 
        ...
        SelPhase = [];
%         for pp = 1:N_Rx*(N_Rx - 1)/2
%             SelPhase = [LoadPhase{((ChosenIndex - 1)*N_Rx*(N_Rx - 1)/2 + pp)};SelPhase];
%         end
%         SelNum = 5; % 选取三条
        WaveLen = 0.45;
        VarMag = 0.05;
        BPM_per_Selphase = zeros(1, N_Rx*(N_Rx - 1)/2);
        if dataPerson(ii) == 1
            for ss = 1:N_Rx*(N_Rx - 1)/2
                SelPhase = LoadPhase{((ChosenIndex - 1)*N_Rx*(N_Rx - 1)/2 + ss)};
                SelPhase = bandpass(SelPhase, fpass, fs(ii));
%                 SelPhase = SelPhase(200:end - 100);
                [timepeak, Index, w_phase, p_phase] = findpeaks(SelPhase); % 时域上峰值
%                 [length(Index),60/(length(SelPhase)/fs(ii)/length(Index))]
                if length(Index) >= 30
                    Index = Index(w_phase >= 0.2*fs(ii)*60/MaxBPM);
                    timepeakNum = length(Index);
                else
                w_phase = w_phase(timepeak >= mean(SelPhase) + VarMag*sqrt(var(SelPhase)));
%                 p_phase = p_phase(timepeak >= mean(SelPhase) + VarMag*sqrt(var(SelPhase)));
                Index = Index(timepeak >= mean(SelPhase) + VarMag*sqrt(var(SelPhase)));
                Index = Index(w_phase >= WaveLen*fs(ii)*60/MaxBPM);
%                 timepeakNum = sum(- Index(1:end-1) + Index(2:end) >= 60/MaxBPM*fs(ii))  + 1;
% Index = Index(p_phase > 0.1);                
timepeakNum = length(Index);
                end
            ...
            for ss = 1:SelNum
                SelPhase1 = LoadPhase{((ChosenIndex - 1)*N_Rx*(N_Rx - 1)/2 + ss)};
                SelPhase1 = bandpass(SelPhase1, fpass, fs(ii));
%                 SelPhase1 = SelPhase1(200:end - 100);
